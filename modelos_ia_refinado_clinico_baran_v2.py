...
def build_engineered_features(df, qmap):
    ...
    for q in [f"Q{i:02d}" for i in range(1, 42)]:
        if q in df.columns:
            out[f"{q}_num"] = df[qmap[q]].apply(cat_to_num)
    ...
    out["normal_entre_crisis"] = df[qmap["Q27"]].apply(is_yes).astype(float)
    out["amanece_bien_luego_aparece"] = df[qmap["Q37"]].apply(is_yes).astype(float)
    ...
    out["giro"] = df[qmap["Q03"]].apply(is_yes).astype(float)
    ...
    for q in migraine_items:
        out[f"{q}_yes"] = df[qmap[q]].apply(is_yes).astype(float)
    ...
    for q in positional_items:
        out[f"{q}_yes"] = df[qmap[q]].apply(is_yes).astype(float)
    out["Q38_mas"] = df[qmap["Q38"]].apply(is_yes).astype(float)
    out["Q38_menos"] = df[qmap["Q38"]].apply(is_alt).astype(float)
    out["Q38_sin_cambio"] = df[qmap["Q38"]].apply(is_no).astype(float)
    out["Q41_mas"] = df[qmap["Q41"]].apply(is_yes).astype(float)
    out["Q41_menos"] = df[qmap["Q41"]].apply(is_alt).astype(float)
    out["Q41_sin_cambio"] = df[qmap["Q41"]].apply(is_no).astype(float)
    ...
    out["no_giro"] = df[qmap["Q03"]].apply(is_no).astype(float)
    out["desmayo"] = df[qmap["Q12"]].apply(is_yes).astype(float)
    out["opresion_pecho_manos"] = df[qmap["Q34"]].apply(is_yes).astype(float)
    out["levantarse_rapido_silla"] = df[qmap["Q36"]].apply(is_yes).astype(float)
    out["quieto_de_pie"] = df[qmap["Q28"]].apply(is_yes).astype(float)
    out["espacios_llenos"] = df[qmap["Q33"]].apply(is_yes).astype(float)
    out["en_movimiento"] = df[qmap["Q35"]].apply(is_yes).astype(float)
    out["quedarse_quieto_menos"] = df[qmap["Q40"]].apply(is_alt).astype(float)
    out["quedarse_quieto_mas"] = df[qmap["Q40"]].apply(is_yes).astype(float)
    out["quedarse_quieto_sin_cambio"] = df[qmap["Q40"]].apply(is_no).astype(float)
    out["periodo_menstrual_mas"] = df[qmap["Q39"]].apply(is_yes).astype(float)
    out["periodo_menstrual_menos"] = df[qmap["Q39"]].apply(is_alt).astype(float)
    return out
...
def make_response_orientation_table(df_raw, outcomes, feature_plan, qmap):
    ...
    for outcome in outcomes:
        ...
        for q in feature_plan[outcome]["raw_cat"]:
            if q not in qmap:
                continue
            s = df_raw[qmap[q]].copy().replace("-", np.nan).astype("object")
            m = s.notna()
            s.loc[m] = s.loc[m].astype(str).str.strip()
            vc = s.value_counts(dropna=True)
...
def main():
    ...
    response_orientation_df = make_response_orientation_table(cohort, outcomes, feature_plan, qmap)
